...
void GraphBuilder::access_field(Bytecodes::Code code) {
  ...
  if (!holder->is_initialized() || needs_patching || compilation()->env()->is_precompiled()) {
...
void GraphBuilder::invoke(Bytecodes::Code code) {
  ...
  if (!PatchALot && Inline && target->is_loaded() && !patch_for_appendix &&
    ...
    if ((code == Bytecodes::_invokestatic && klass->is_initialized() && !compilation()->env()->is_precompiled()) || // invokestatic involves an initialization barrier on declaring class
        code == Bytecodes::_invokespecial ||
...
bool GraphBuilder::try_inline_full(ciMethod* callee, bool holder_known, bool ignore_return, Bytecodes::Code bc, Value receiver) {
  ...
  if (bc == Bytecodes::_invokestatic &&
      (!callee->holder()->is_initialized() || !compilation()->env()->is_precompiled())) INLINE_BAILOUT("callee's klass not initialized yet");
